...
def get_movies(genres_ids: list[int] = None, actors_ids: list[int] = None) -> list[Movie]:
    if genres_ids and actors_ids:
        return Movie.objects.filter(genres__in=genres_ids).filter(actors__in=actors_ids)
    if genres_ids:
...
def create_movie(movie_title: str, movie_description: str, genres_ids: list[int] = None, actors_ids: list[int] = None) -> None:
