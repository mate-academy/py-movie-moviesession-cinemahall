...
def update_movie_session(session_id: int,
    ...
    movie_query = MovieSession.objects.filter(id=session_id)

    if show_time:
        movie_query.update(show_time=show_time)

    if movie_id:
        movie_query.update(movie_id=movie_id)

    if cinema_hall_id:
        movie_query.update(cinema_hall_id=cinema_hall_id)
