...
def update_movie_session(
    ...
    upd_movie_session = MovieSession.objects.filter(
        id=session_id
    )

    if show_time:
        upd_movie_session.update(show_time=show_time)

    if movie_id:
        upd_movie_session.update(movie_id=movie_id)

    if cinema_hall_id:
        upd_movie_session.update(cinema_hall_id=cinema_hall_id)
