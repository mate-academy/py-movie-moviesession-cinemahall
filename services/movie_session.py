from django.db.models import QuerySet
from datetime import datetime
from db.models import MovieSession

def update_movie_session(session_id: int,
                         show_time: datetime = None,
                         movie_id: int = None,
                         cinema_hall_id: int = None,
                         ):
    movie_session_to_update = MovieSession.objects.get(id=session_id)

    if movie_id:
        MovieSession.objects.filter(id=session_id).update(movie_id=movie_id)
    if cinema_hall_id:
        MovieSession.objects.filter(id=session_id).update(cinema_hall_id=cinema_hall_id)
    if show_time:
        MovieSession.objects.filter(id=session_id).update(show_time=show_time)
    # if show_time:
    #     movie_session_to_update.show_time = show_time
    # if movie_id:
    #     movie_session_to_update.session_to_movies.set(movie_id)
    # if cinema_hall_id:
    #     movie_session_to_update.session_to_cinema_halls.set(cinema_hall_id)
    return movie_session_to_update


def delete_movie_session_by_id(session_id: int) -> None:
    movie_session_to_delete = MovieSession.objects.get(id=session_id)
    movie_session_to_delete.delete()








def create_movie_session(movie_show_time: datetime,
           movie_id: int,
           cinema_hall_id: int) -> MovieSession:
    new_movie_session = MovieSession.objects.create(
        show_time=movie_show_time,
        movie_id=movie_id,
        cinema_hall_id=cinema_hall_id
       )
    return new_movie_session

def get_movie_session_by_id(movie_session_id: int) -> MovieSession | None:
    result = MovieSession.objects.get(id=movie_session_id)
    return result

def get_movies_sessions(
        session_date: datetime = None,
) -> QuerySet | None:
    queryset = MovieSession.objects.all()
    if session_date:
        queryset = queryset.filter(show_time__date=session_date)
    return queryset